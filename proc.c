...
int getprocesstimedetails(int pid)
{
  ...
  for(cp = ptable.proc; cp < &ptable.proc[NPROC]; cp++){
    if(cp->pid == pid)
    {
      if(!(&(cp->d1)) || !(&(cp->d2))  || !(&(cp->d3)))
      {
        return -1;
      }
    else
    {
      cprintf("Creation Time: %d:%d:%d %d:%d:%d \n",(cp->d1).second,(cp->d1).minute,(cp->d1).hour,(cp->d1).day,(cp->d1).month,(cp->d1).year);
      cprintf("Last Context Switched In Time: %d:%d:%d %d:%d:%d \n",(cp->d2).second,(cp->d2).minute,(cp->d2).hour,(cp->d2).day,(cp->d2).month,(cp->d2).year);
      cprintf("Last Context Switched Out Time: %d:%d:%d %d:%d:%d \n",(cp->d3).second,(cp->d3).minute,(cp->d3).hour,(cp->d3).day,(cp->d3).month,(cp->d3).year);
      return 1;
    }
    }
